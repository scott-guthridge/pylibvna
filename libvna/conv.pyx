...
#
# fn_2x2_renormalize: pointer to function taking two double complex [2][2]
#                     matrices and two vectors of z0 values
#
ctypedef void (*fn_2x2_renormalize)(const double complex (*in_2x2)[2],
                                    double complex (*out_2x2)[2],
...
#     of z0
#
ctypedef void (*fn_NxN_renormalize)(const double complex *in_NxN,
                                    double complex *out_NxN,
                                    const double complex *z1,
                                    const double complex *z2, int n)
...
#
# check_renormalize: check for z1, z2 both None or both not None
#                    return True if they're given
#
cdef check_renormalize(name, z1, z2):
    if z1 is None and z2 is None:
        return False
    if z1 is not None and z2 is not None:
        return True
    raise ValueError(name + ': z1 and z2 must both be given or must '
                            'both be None')
...
#
# helper_2x2_renormalize: call fn on each 2x2 array in input
#
cdef helper_2x2_renormalize(fn_2x2_renormalize fn, name, array, z1, z2):
    #
    # Convert the input array to C compatible double complex form,
    # validate that the last two dimensions are 2x2, and create the
    # output array with same shape as input.
    ...
    #
    # Convert z1 and z2 to C compatible complex m x 2, where m is the
    # number of outer elements.
    #
    z1 = canonicalize_z0(z1, name, input.shape[:-2], 2)
    z2 = canonicalize_z0(z2, name, input.shape[:-2], 2)
    cdef double complex [:, :] cz1 = z1
    cdef double complex [:, :] cz2 = z2
...
#
# helper_NxN_renormlize: call fn on each NxN array in input
#
cdef helper_NxN_renormalize(fn_NxN_renormalize fn, fn_2x2_renormalize fn2,
                            name, array, z1, z2):
    #
    # Convert the input array to C compatible double complex form,
    # validate that the last two dimensions are square, and create
    ...
    # number of outer elements.
    #
    z1 = canonicalize_z0(z1, name, input.shape[:-2], n)
    z2 = canonicalize_z0(z2, name, input.shape[:-2], n)
    cdef double complex [:, :] cz1 = z1
    cdef double complex [:, :] cz2 = z2
    ...
    if n == 2:
        ...
        for i in range(vin.shape[0]):
            fn(&cvin[i, 0, 0], &cvout[i, 0, 0], &cz1[i][0], &cz2[i][0], n)
...
###############################################################################
# Conversions That Can Re-Normalize
###############################################################################

def stos(array, z1, z2):
    '''
    Re-normalize scattering parameters.
    '''
    return helper_NxN_renormalize(&vnaconv_stosrn, &vnaconv_stosr, 'stos',
                                  array, z1, z2)


def stot(array, z1=None, z2=None):
    '''
    Convert scattering parameters to scattering-transfer parameters.
    '''
    if check_renormalize('stot', z1, z2):
        return helper_2x2_renormalize(&vnaconv_stotr, 'stot', array, z1, z2)

    return helper_2x2(&vnaconv_stot, 'stot', array)


def stou(array, z1=None, z2=None):
    '''
    Convert scattering parameters to inverse scattering-transfer parameters.
    '''
    if check_renormalize('stou', z1, z2):
        return helper_2x2_renormalize(&vnaconv_stour, 'stou', array, z1, z2)

    return helper_2x2(&vnaconv_stou, 'stou', array)


def ttos(array, z1=None, z2=None):
    '''
    Convert scattering-transfer parameters to scattering parameters.
    '''
    if check_renormalize('ttos', z1, z2):
        return helper_2x2_renormalize(&vnaconv_ttosr, 'ttos', array, z1, z2)

    return helper_2x2(&vnaconv_ttos, 'ttos', array)


def ttot(array, z1, z2):
    '''
    Renormalize scattering-transfer parameters.
    '''
    return helper_2x2_renormalize(&vnaconv_ttotr, 'ttot', array, z1, z2)


def ttou(array, z1=None, z2=None):
    '''
    Convert scattering-transfer parameters to inverse scattering-transfer
    parameters.
    '''
    if check_renormalize('ttou', z1, z2):
        return helper_2x2_renormalize(&vnaconv_ttour, 'ttou', array, z1, z2)

    return helper_2x2(&vnaconv_ttou, 'ttou', array)


def utos(array, z1=None, z2=None):
    '''
    Convert inverse scattering-transfer parameters to scattering
    parameters.
    '''
    if check_renormalize('utos', z1, z2):
        return helper_2x2_renormalize(&vnaconv_utosr, 'utos', array, z1, z2)

    return helper_2x2(&vnaconv_utos, 'utos', array)


def utot(array, z1=None, z2=None):
    '''
    Convert inverse scattering-transfer parameters to scattering-transfer
    parameters.
    '''
    if check_renormalize('utot', z1, z2):
        return helper_2x2_renormalize(&vnaconv_utotr, 'utot', array, z1, z2)

    return helper_2x2(&vnaconv_utot, 'utot', array)


def utou(array, z1, z2):
    '''
    Renormalize inverse scattering-transfer parameters.
    '''
    return helper_2x2_renormalize(&vnaconv_utour, 'utou', array, z1, z2)
